...
def _pick_state(sess: dict) -> dict:
    ...
    return {
        ...
        # ★«찍은 헤드가 몇 개인가» — 화면이 물어도 답할 데가 없던 수다.
        #
        #   `n_heads` 는 헤드 **칸(부류)** 수다(레이어×색×반지름). 실측
        #   대명동 평면도: 칸 3개 · 실제 헤드 111개. 그런데 찍기의 클릭
        #   하나가 칸을 통째로 끄므로, 칸 하나만 꺼도 헤드가 111 → 5 로
        #   떨어진다. 그 사실을 찍기 화면에서 볼 수 없어 사람은 한참 뒤
        #   「최불리 선정이 안 된다」로 만난다.
        #
        #   여기서 세는 것은 강조에 쓰는 «찍힌 헤드 원» 그대로다 — 새 셈을
        #   만들지 않는다. 삼각형 헤드는 원이 아니라 별도라 이 수에 없다.
        #   그래서 있으면 있다고 알린다(없는 수를 지어내지 않는다).
        "n_head_circles": len(hl["head_circles"]),
        "has_tri_heads": any("tri_side" in h for h in ps.board.heads),
    }

...
# ═══════════════════════════ 요구 한 줄 — 선정 == 표
#
#   사용자 지시(2026-09-13): 「간단히, 최불리 선정으로 나온 헤드 개수랑 표
#   확정으로 나온 헤드 개수와 좌표가 정확히 똑같아지게만 조치해 달라.」
#
#   갈리던 자리는 하나뿐이었다 — **순서가 거꾸로**였다:
#       손질    : 붙는지 안 보고 K개를 고른다
#       수리계산 : cand = 고른 것 ∩ 붙는 헤드 → 모자라면 다음 순위로 채운다
#   그래서 손질이 「헤드 30」이라 적어 두고 표에는 다른 헤드가 왔다.
#
#   조치 뒤 실측(scripts/_probe_pick_equals_table.py · 대명동):
#       K=10 영역0 · K=30 영역0 · K=10 영역2 · K=30 영역3 — 전부 1:1 전원 일치
#       B1F K=30 : 30/30 · 선정에만 0 · 표에만 0
def test_손질이_붙는_헤드만_후보로_삼는다():
    """★이것이 «개수·좌표가 같다» 를 **구성으로** 보장하는 자리다."""
    s = _src("routes/module_f/api_edit.py")
    i = s.index("probe = wet_heads(")
    seg = s[i:i + 1000]
    assert "only = wet if only is None else (set(only) & wet)" in seg
    # 못 쟀다고 막으면 고칠 길이 사라진다.
    assert "종전대로 고릅니다" in seg


def test_탐침_캐시_키를_수리계산과_맞춘다():
    """★안 맞추면 같은 판을 두 번 전개한다 — B1F 실측 88.4s 가 두 번(188s).

    `/design/build` 는 급수원이 하나뿐이면 `selected_source=None` 을 넘긴다.
    손질이 "Z1" 을 넘기면 키가 갈려 캐시가 조용히 빗나간다. 조용히 빗나가면
    비용만 늘고 아무도 모른다 — 고친 뒤 표 확정이 **1.8s** 로 떨어졌다.
    """
    s = _src("routes/module_f/api_edit.py")
    i = s.index("probe = wet_heads(")
    seg = s[i:i + 400]
    assert "len(b.sources) > 1" in seg, "급수원 1곳일 때 키가 갈린다"
    d = _src("routes/module_f/api_design.py")
    j = d.index("probe = wet_heads(sess, es, selected_source=sel)")
    assert j > 0, "수리계산 쪽 호출이 바뀌었다 — 키 규약을 다시 맞출 것"


def test_채움은_안전망으로만_남긴다():
    """★백필을 지우지는 않는다 — 탐침을 못 쟀을 때의 마지막 그물이다.

    이제 정상 경로에서는 `short == k_use` 라 걸리지 않는다(실측 4조합 + B1F).
    """
    s = _src("routes/module_f/api_design.py")
    i = s.index("filled = 0")
    seg = s[i:i + 1200]
...
def test_화면이_첫_선정이_길다고_말한다():
    """얼마나 걸릴지 안 말하면 «멈췄다» 로 읽힌다(B1F 첫 회 88초)."""
    js = _src("static/module_f.js")
    i = js.index('$("ed-worst").onclick')
    seg = js[i:i + 300]
    assert "처음 한 번만" in seg, "첫 회 비용을 안 말한다"

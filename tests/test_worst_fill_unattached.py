...
def test_손질이_붙는_헤드만_고른다():
    """★이 시험은 **전제가 뒤집혀** 다시 썼다(2026-09-13). 이력을 남긴다.

    종전 이름은 `test_손질은_전개를_돌리지_않는다` 였고, 「손질에서 재면
    화면이 2분 얼어붙는다(B1F 117초) · 총합은 같고 자리만 옮긴다」를 근거로
    **탐침이 없는 것**을 감시했다.

    사용자 지시로 그 결정을 뒤집었다 — 「최불리 선정으로 나온 헤드 개수·좌표와
    표 확정의 그것이 정확히 똑같아지게」. 문제는 총합(비용)이 아니라 **답**
    이었다: 붙는지 안 보고 고르면 표가 뒤에서 다른 헤드로 채우고, 손질이
    「헤드 30」이라 적어 둔 것과 표의 30개가 달라진다.

    그러니 이제 감시할 것은 «탐침이 없다» 가 아니라 **«붙는 헤드만 고른다»**
    이다. 비용은 그대로다 — 수리계산이 어차피 내던 값이고 판마다 한 번만
    재는 캐시를 그대로 쓴다.
    """
    s = _src("routes/module_f/api_edit.py")
    assert "wet_heads(" in s, "손질이 붙는 헤드를 안 본다"
    # ★닻은 «호출이 있다» 로 잡는다 — 인자 모양을 통째로 박아 두면 급수원
    #   캐시 키를 맞추느라 줄바꿈만 해도 깨진다(실제로 깨뜨렸다).
    i = s.index("probe = wet_heads(")
    seg = s[i:i + 1100]
    assert "only = wet if only is None else (set(only) & wet)" in seg
    # 못 쟀다고 막으면 고칠 길이 사라진다 — 종전대로 고르고 말한다.
    assert "종전대로 고릅니다" in seg
    # 뒤집은 이유가 적혀 있어야 다음 사람이 또 뒤집지 않는다.
    assert "2026-09-13" in s and "117초" in s

...
# core/ 공유 모듈을 flat-name 으로 import 하기 위한 sys.path shim (Phase2b 코어 추출).
_CORE_DIR = BASE_DIR / "core"
for _p in (str(BASE_DIR), str(_CORE_DIR)):
...
from dxf_parse_progress import parse_dxf_with_progress
...
def _bake_isometric_node_coords(nodes: list[dict], iso_z_scale: float = 1.0,
    ...
    AV 한 점에서 찢어진다(대명동 실측 도면 대각의 18%). 영점을 bbox 중앙이 아니라
    AV 표고로 잡으면 이음매에서 lift 가 정확히 0 이 되어 양쪽 밸브가 겹친다.
    미지정이면 종전대로 bbox 중앙.
    """
    if not nodes:
        return

...
def test_정적_파일_캐시_표가_한_벌로_움직인다():
    """★한쪽만 올리면 사람 브라우저가 **옛 화면 코드**를 그대로 쓴다.

    실제로 한 번 그랬다 — CSS 만 올리고 JS 를 안 올려서, 새 단추가 든 JS 가
    캐시에 막혀 안 내려갈 뻔했다. Playwright 는 캐시가 비어 있어 통과한다.
    그래서 «새 창에서 확인했다» 로는 이 결함이 안 잡힌다.

    무엇이 바뀌었는지까지는 시험이 알 수 없다. 대신 **둘이 같은 표를 달게**
    한다 — 하나를 올리면 다른 하나도 올리게 되는 자리다.
    """
    vs = re.findall(r'/static/module_f\.(?:css|js)\?v=([\w-]+)', _html())
    assert len(vs) == 2, f"화면이 부르는 정적 파일이 둘이 아니다: {vs}"
    assert vs[0] == vs[1], f"캐시 표가 서로 다르다: {vs}"

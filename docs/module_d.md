...
## 6. 헤드는 배관에 매달려 있다 — 노즐 스텁

노즐도 입력노드와 출력노드를 잇는 링크다. 그 선을 그리지 않으면 헤드 표시만
허공에 떠 관로와 무관해 보인다. 두 가지를 색으로 갈라 그린다.

| 자리의 출처 | 선 | 헤드 표시 |
|---|---|---|
| 원본 `@/n` 좌표 | 관로와 같은 회색 실선 `#333333` | 검은 삼각 |
| 우리가 유도한 방향 | 주황 점선 `#c2410c` | 흰 삼각 + 주황 테두리 |

지어낸 선을 실측 선과 같은 색·같은 실선으로 그리면 도면을 보는 사람이 둘을
구분할 길이 없다. 유도한 개수는 `RenderReport.nozzles_derived` 와 경고 문구에도
전량 실린다.

스텁 길이는 방향이 없을 때만 쓰는 값이다(`_STUB_SPAN_RATIO = 0.01208`, 코퍼스
SDF 334 개 / 노즐 3437 개 중앙값). 대명동 수작업본 한 파일만 재면 0.01747 로
더 길지만, 한 파일보다 334 파일 중앙값의 근거가 넓어 바꾸지 않았다.

## 7. 수치 일치 — 실측 (§2.2 수용 기준)
...
| 자릿수 경계 불일치 | XML 정밀도 자체의 한계 | §7 처럼 폭을 재서 정밀도 안임을 보인다. 값을 손보지 않는다 |

## 11. 검증 자산

- `tests/test_module_d/` — 106 passed.

...
두 가지를 못박는다:
  ...
  ② far_m 이 «어느 줄» 인지가 나온다.
     ...
     화면에서 읽을 수 없었다. anchor_path 가 그 줄이다.
"""
...
def test_앵커는_가장_먼_헤드다():
    ...
    assert w["anchor"] == len(hnodes) - 1          # 맨 끝 헤드
...
def test_최원_경로가_급수원에서_앵커까지다():
    pts, edges, hnodes, src = _line(6)
    w = worst_k_heads(pts, edges, hnodes, src, k=3)
    assert w["anchor_path"] == [0, 1, 2, 3, 4, 5]
    assert w["anchor_path_m"] == 5.0
...
def test_최원_경로_길이가_far_m와_같다():
    ...
    assert math.isclose(w["anchor_path_m"], w["far_m"], abs_tol=0.01)


def test_경로는_급수원에서_시작한다():
    pts, edges, hnodes, src = _line(6)
    w = worst_k_heads(pts, edges, hnodes, src, k=3)
    assert w["anchor_path"][0] == 0


def test_헤드가_없으면_경로도_빈다():
    pts, edges, hnodes, src = _line(4)
    w = worst_k_heads(pts, edges, hnodes, src, k=3, only_heads=set())
    assert w["heads"] == [] and w["anchor_path"] == []
    assert w["anchor_path_m"] == 0.0
...
def test_영역_안_경로도_급수원까지_이어진다():
    ...
    assert w["anchor"] == 5
    assert w["anchor_path"][0] == 0            # 영역 밖을 지나서라도 급수원까지
    assert len(w["anchor_path"]) == 7          # 노드 0..6

...
# 밸브 되짚기 허용치. 값은 `tables.WORST_HEAD_SNAP_M` 과 같지만 **근거가 다르므로
# 한 상수로 묶지 않는다.**
#
#   기준 헤드 쪽 — 후보가 «헤드 노드» 뿐이라 성기다(실측 이웃 2.95m). 허용치를
#                  넉넉히 잡아도 이웃을 집을 수가 없다.
#   여기(밸브) — 후보가 헤드 아닌 **모든** 노드라 훨씬 빽빽하다. 실측(B1F)으로
#                17mm 떨어진 노드가 함께 걸렸다. 그러니 「이웃과 멀다」는 논거를
#                여기에 그대로 쓰면 안 된다.
#
# 그래도 100mm 가 안전한 이유는 다르다: 되짚기의 출발점이 **그 밸브 자신의
# board 좌표** 라, 옳은 답까지의 거리가 노드정리의 자리 이동분(실측 6~8mm)뿐이다.
# 더 가까운 다른 노드가 있을 수 없다. 허용치는 그 이동분을 덮기만 하면 된다.
# (접속점이 함께 걸리는 경우는 거리가 아니라 «접속점 우선» 으로 가른다 — 아래.)
VALVE_SNAP_M = 0.10

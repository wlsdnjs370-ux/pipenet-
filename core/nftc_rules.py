...
def reference_count_options() -> list[dict[str, Any]]:
    """기준개수 표를 «고를 수 있는 목록» 으로 — 화면이 이것으로 표를 그린다.

    `decide_reference_count` 는 건물 사실(용도·층수·부착높이…)을 받아 한 행을
    고른다. 그런데 모듈 F 의 손질 단계에는 그 사실이 아직 없다 — 도면만 있다.
    그래서 **사람이 표에서 직접 고르게** 한다.

    표를 화면 쪽에 옮겨 적지 않는다. 법정 수치를 두 곳에 두면 개정이 왔을 때
    한쪽만 고쳐지고, 그 어긋남은 기준개수가 30 이어야 할 자리에 20 이 들어간
    산출로만 드러난다 — 여기가 유일한 출처다(NFTC 103 표 2.1.1.1).

    반환은 표 순서 그대로다. 순서가 곧 규칙이라(먼저 맞는 행이 이긴다) 화면도
    같은 순서로 보여야 사람이 «왜 이 행인가» 를 표와 대조할 수 있다.
    """
    return [{"rule_id": row["rule_id"], "label": row["label"],
             "count": int(row["count"])}
            for row in _REFERENCE_COUNT_TABLE]

...
def test_평면_밑그림에서_그_자리_수정된다():
    ...
    ★[2026-09-08] 다만 «이음·삭제» 를 고른 동안에만 고친다. 평면 보기가
      기본이 된 뒤로는 손질에서 쓰던 모드(알람밸브 등)가 그대로 남아 있을 수
      있는데, 그때 무심코 찍으면 밸브가 놓인다 — 보기 화면에서 일어나면 안
      되는 일이다. 그래서 조건이 한 줄 길어졌고, 잘라 보는 창도 넓혔다.
    """
    html = _screen()
    i = html.index('S.stage === "design" && planUnderlayOn()')
    seg = html[i:i + 600]
    assert "editClick(" in seg, "설계 화면 클릭이 손질 경로를 안 탄다"
    assert '"이음"' in seg and '"삭제"' in seg, "고르지 않아도 고쳐진다"

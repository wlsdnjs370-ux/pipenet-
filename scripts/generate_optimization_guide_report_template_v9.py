...
TEMPLATE = ROOT / "templates_hwp" / "template_optimization_guide_from_user_v9.hwp"
OUT = DESKTOP / "PIPENET_2_optimization_guide_on_template_v9_VISIBLE_FIRST.hwp"
OUT_KR = DESKTOP / "PIPENET 수리계산 검증 프로그램 중 2. 설계 최적화 가이드 로직_원본양식_본문먼저_v9.hwp"
OUT_WORK = DESKTOP / "PIPENET_2_optimization_guide_on_template_v9_WORK.hwp"
...
def table(title: str, headers: list[str], rows: list[list[str]]) -> str:
    out = [f"\r\n[표] {title}", "\t".join(headers)]
    out.extend("\t".join(row) for row in rows)
    return "\r\n".join(out) + "\r\n"


def flow(title: str, steps: list[str]) -> str:
    out = [f"\r\n[삽도] {title}"]
...
def report_text() -> str:
    ...
    parts.append(
        "Analysis Report\r\n"
        "PIPENET 수리계산 검진 프로그램 보고서\r\n"
        "\r\n"
        "2. 설계 최적화 가이드 로직 보고서\r\n"
        "\r\n"
        "본 문서는 사용자가 제공한 원본 HWP 양식 위에서 작성한 보고서이다. "
        "PIPENET 수리계산 검증 프로그램의 2. 설계 최적화 가이드가 어떤 입력값을 읽고, "
        "어떤 수식과 판정 기준을 거쳐 공학적 최적화 후보와 시공사 경제성 후보를 제시하는지 설명한다.\r\n"
    )
    parts.append(
        table(
            "목차",
    ...
    parts.append(section("1. 문서 목적 및 기본 전제"))
    parts.append(
        "설계 최적화는 법적 기준을 무시하고 관경을 줄이는 작업이 아니다. "
        "공학안과 경제안 모두 필수 검증 기준을 통과한 설계를 전제로 하며, "
        "이후 안정성 또는 비용 절감 관점의 개선 후보를 분리해 제시한다.\r\n"
    )
    parts.append(
        table(
            "필수 PASS 전제 기준",
    ...
    parts.append(
        flow(
            ...
            [
                "입력: 결과서 DOCX/PDF, SDF 배관망, project_meta, design_policy",
                "파싱: 배관, 헤드, 압력, 유속, 손실, C-Factor, 토폴로지 구조화",
                "필수 기준 PASS 확인: 법적/기술 기준 미달 시 최적화보다 수정 우선",
                "공학/경제 후보 분리: 안정성 개선 후보와 비용 절감 후보를 별도 산출",
                "출력: 결과 데이터 테이블, 공학/경제 맵, 상세 카드, 최종 리포트",
            ],
        )
    )
    parts.append(section("2. 입력 데이터와 파서 구조"))
    parts.append(
        table(
            "입력 데이터와 사용 목적",
            ["입력", "주요 추출 필드", "역할"],
    ...
    parts.append(section("3. 공통 계산 지표와 수식"))
    parts.append(
        table(
            "공통 계산 지표와 수식",
            ["지표", "수식", "해석"],
    ...
    parts.append(section("4. 공학적 마찰손실 최적화 로직"))
    parts.append(
        "공학적 최적화는 안정성, 압력 여유, 마찰손실 분산, 유속 안정성을 우선한다. "
        "비용 절감보다 특정 배관에 손실이 집중되는 현상과 말단 압력 불안정을 줄이는 것이 목적이다.\r\n"
    )
    parts.append(
        flow(
            "공학적 마찰손실 최적화 판정 흐름",
            [
                "지표 계산: m당 손실, 변화율, 유속 여유율, 압력 여유, 등가길이 비율",
                "후보 세분화: 마찰손실, 변화율, 유속여유, 피팅집중, 압력여유",
    ...
    parts.append(section("5. Friction Loss Spike Map"))
    parts.append(
        table(
    ...
    parts.append(section("6. 시공사 경제성 확보 로직"))
    parts.append(
        "경제성 최적화는 가능한 싸게 만드는 것이 아니라, 법적/기술 기준을 유지하는 범위에서 과설계를 줄이는 방향이다. "
        "관경 축소 후에도 유속, 말단압력, 헤드 유량, HW 검산 조건을 재확인해야 한다.\r\n"
    )
    parts.append(
        flow(
            "시공사 경제성 확보 판정 흐름",
            [
                "후보 탐색: 저유속, 압력여유 과다, 대구경 밸브, CPVC 대구경, 과대 관경",
                "축소 가정: 관경 한 단계 축소를 가정하고 재계산 준비",
                "HW 재계산: 축소 후 마찰손실, 유속, 말단압력, 헤드 유량 검토",
                "통과 조건: 가지 ≤ 6 m/s, 그 밖 ≤ 10 m/s, 방수압 ≥ 0.1 MPa",
                "경제 제안: 관경 축소, 밸브 100A 이하 조정, CPVC 대구경 회피",
            ],
        )
    )
    parts.append(
    ...
    parts.append(section("7. Economy Optimization Candidate Map"))
    parts.append(
        table(
    ...
    parts.append(section("8. 결과 데이터 테이블 표시 구조"))
    parts.append(
        table(
            "결과 데이터 테이블 열 그룹",
            ["열 그룹", "주요 필드", "클릭 시 설명"],
    ...
    parts.append(section("9. 공학-경제 충돌 및 절충 판단"))
    parts.append(
        table(
            "공학-경제 충돌 및 절충 판단",
    ...
    parts.append(section("10. 관경 축소 시뮬레이션 흐름"))
    parts.append(
        table(
            "관경 축소 시뮬레이션 절차",
            ["단계", "처리 내용", "통과 조건"],
    ...
    parts.append(section("11. 한계 및 추가 입력"))
    parts.append(
        table(
            "한계 및 추가 입력",
            ["항목", "현재 한계", "보완 입력"],
    ...
    parts.append("\r\n\r\n※ 이하에는 사용자가 제공한 원본 HWP 양식의 기존 표지/문서목적/목차 영역이 보존되어 있다.\r\n")
    return "".join(parts)


def build() -> None:
    shutil.copy2(TEMPLATE, OUT_WORK)
    ...
    try:
        try:
            ...
            pass
        if not hwp.Open(str(OUT_WORK), "HWP", "forceopen:true"):
            raise RuntimeError("open failed")
        hwp.HAction.Run("MoveDocBegin")
        clipboard(report_text())
        ...
        hwp.HParameterSet.HFileOpenSave.filename = str(OUT)
        hwp.HParameterSet.HFileOpenSave.Format = "HWP"
        hwp.HParameterSet.HFileOpenSave.Attributes = 0
        saved = bool(hwp.HAction.Execute("FileSaveAs_S", hwp.HParameterSet.HFileOpenSave.HSet))
        if not saved:
            hwp.Save()
    finally:
        hwp.Quit()
    if not OUT.exists() and OUT_WORK.exists():
        shutil.copy2(OUT_WORK, OUT)
    shutil.copy2(OUT, OUT_KR)


def validate() -> None:
    text = subprocess.check_output(["hwp5txt.exe", str(OUT)], text=True, encoding="utf-8", errors="replace", timeout=30)
    markers = ["Analysis Report", "2. 설계 최적화", "목차", "공학적 마찰손실", "시공사 경제성", "사용자가 제공한 원본 HWP"]
    print(f"OUT={OUT} exists={OUT.exists()} size={OUT.stat().st_size if OUT.exists() else 0}")

...
■ 골든 이력 — 왜 움직였는지 적어 둔다 (안 적으면 «덮었다» 로 읽힌다)
  ...
  2026-09-14 · `daemyeong` **네 칸** — 사용자 승인 후 기록.

        board      2,770 / 2,357 → 2,782 / 2,369   (+12 · 헤드 수는 133 그대로)
        full         746 /   745 →   878 /   877
        worst_kfp     79 /    78 →    89 /    88
        worst.total_m      74.02 →     74.39 m

    +12 은 **§2-2**(`ModuleF_최불리규칙_복원_지시서.md`)가 「관이 헤드를 스쳐
    지나가는」 헤드 12개를 붙인 것이다 — 종전에는 그 헤드들을 최불리 후보에서
    빼서 없는 척했고, 하필 가장 먼 헤드가 그 모양이라 「먼 순서 그대로 K 개」가
    뒤집혔다(실측: 상위 30 중 5개가 그렇게 빠졌다). 붙였으므로 board 가 늘고
    전체망·최불리 `.kfp` 가 그만큼 커졌다.

    `far_m 52.69` · `span_m 11.10` · `max_load 10` 은 **안 움직였다** — 최불리
    지점 자체는 그대로다. `total_m` +0.37 m 는 새로 붙인 접속관 길이다.

    ★**b1f 5칸은 한 칸도 안 움직였다**(저장본 지문이 기준선과 달라 그 구간이
      스스로 건너뛴다 — 이건 이 작업과 무관한 선재 조건이다).
    ★회랑 사슬좌표(`ModuleF_회랑_사슬좌표_지시서.md`)는 이 값들을 **안 바꿨다** —
      사슬은 `expand_worst` 안에서만 돌고 전체망은 그 경로를 안 탄다.
      실측으로 확인: 사슬 넣기 전·후 모두 전체망 878/877 · 1,636,292 bytes.

    ★되돌리기: `data/_golden_backup_20260914.json`

...
■ ★2026-09-13 이 판단은 뒤집혔다 — 손질이 «고르기 전에» 거른다

  아래 「왜 손질에서 미리 안 거르나」 는 당시의 판단이고, 그 대가로 손질이
  고른 것과 표에 온 것이 **다른 헤드**가 되었다(평면과 아이소가 다른 그림).
  사람이 손질에서 정의한 것이 정본이므로(`ModuleF_손질정본_지시서.md` §0)
  거르는 자리를 손질로 옮겼다. 속도는 실제로 치렀다 — B1F 최불리 0.3s → 93s.
  백필은 **안전망으로 남는다**(없애면 K 가 다시 무너진다).

■ 왜 손질에서 미리 안 거르나 — 재 보고 접었다 (당시 기록)
...
def test_손질이_뽑은_뒤에_그_K개만_전개한다():
    """★두 번 뒤집혔다 — 지금 자리가 세 번째다.

        2026-09-09  손질은 전개를 **안 돈다** (빠르게 · 수리계산이 뒤에서 채움)
        2026-09-13  고르기 **전에** 전체망을 전개해 후보를 거른다
                    → 규칙이 깨졌다(먼 헤드가 빠짐) · B1F 최불리 93초
        2026-09-14  후보는 **안 깎고**, 고른 **뒤에** 그 K 개만 전개한다
                    → 규칙도 서고 빠르다

    셋째가 맞는 이유는 재 보면 나온다. B1F K=30 실측::

        판 열기 0.80s · ★전체망 전개 154.10s(98.2%) · 선정 0.44s
        먼 순서 검사 0.51s · ★제한 전개(K개만) 1.13s

    전개를 재는 이유는 §2-3 하나 — **뽑힌 K 개**가 배관에 붙는가. 그 판정에
    도면 전체를 전개할 이유가 없다. 답이 같은지도 재 봤다
    (`scripts/_probe_picked_wet_agrees.py`): 대명동·B1F 둘 다 **같고**,
    B1F 는 175.47s → 0.91s (194배).
    """
    s = _src("routes/module_f/api_edit.py")
    body = s[s.index("def _compute_worst"):]
    i_pick = body.index("w = _worst_k_heads(")
    i_wet = body.index("picked_heads_wet(")
    assert i_pick < i_wet, "고르기 전에 전개하면 후보가 깎인다"
    # 고르기 앞 구간에 전체망 전개가 없어야 한다.
    assert "wet_heads(" not in body[:i_pick], "고르기 전에 전체망을 전개한다"
    # 뽑힌 것만 넘긴다 — 도면 전체가 아니다.
    seg = body[i_wet:i_wet + 300]
    assert 'w["heads"]' in seg, "뽑힌 K 개를 안 넘긴다"
...
def test_모자라지_않으면_손질_선정을_그대로_쓴다():
    ...
    ★**닻과 닻 사이**로 자른다(2026-09-11). 종전에는 «앞에서 N자» 로 잘랐는데,
      그 구간에 새 갈래가 들어올 때마다(§2-5 채우지 않기 · 영역 가두기) 확인할
      줄이 창 밖으로 밀려 시험이 깨졌다. 두 번 넓히고 나서 고쳤다 —
      **창 크기가 시험의 참·거짓을 가르면 안 된다.** 지키려는 것은
      «조건 안에서만 `only` 를 바꾼다» 이지 «몇 자 안에 있다» 가 아니다.
    """
    s = _src("routes/module_f/api_design.py")
    i = s.index("filled = 0")

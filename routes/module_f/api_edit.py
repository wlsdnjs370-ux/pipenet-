...
def register(app):
    ...
    def _compute_worst(sess, body):
        ...
        # ★★★«붙는 헤드» 만 후보로 삼는다 — 순서를 바로잡는다 (2026-09-13).
        #
        #   사용자 지시: 「최불리 선정으로 나온 헤드 개수·좌표와 표 확정으로
        #   나온 헤드 개수·좌표가 정확히 똑같아지게만 해 달라.」
        #
        #   그 둘이 갈리던 자리는 하나뿐이었다 — 순서가 거꾸로였다:
        #       손질   : 붙는지 안 보고 K개를 고른다
        #       수리계산: cand = 고른 것 ∩ 붙는 헤드 → 모자라면 다음 순위로 채움
        #   그래서 손질이 「헤드 30」이라 적어 놓고 표에는 **다른 헤드**가 왔다.
        #
        #   ★이 자리는 2026-09-09 에 «재 보고 접었던» 자리다. 그때 근거는
        #     「그 판정이 곧 전체망 전개 한 번(B1F 실측 117초)이고, /edit/worst
        #     는 동기 요청이라 화면이 2분 얼어붙는다. 총합은 어차피 같고 자리만
        #     옮긴다」였다. 지금 그 결정을 뒤집는 이유는 **총합이 아니라 답이
        #     문제**이기 때문이다 — 자리를 옮기면 두 화면의 헤드가 같아진다.
        #     비용은 그대로다(수리계산이 어차피 내던 값이고, 판마다 한 번만
        #     재는 캐시를 그대로 쓴다). 화면은 가림막에 「최불리 헤드 선정 중」
        #     을 띄운 채 기다린다 — 조용히 멈춘 것이 아니다.
        #
        #   탐침이 실패하면 **막지 않는다.** 못 쟀다고 선정을 못 하게 하면
        #   고칠 방법이 사라진다 — 종전대로 고르고, 수리계산이 뒤에서 채운다.
        from routes.module_f.attach import wet_heads
        # ★캐시 키를 수리계산과 **똑같이** 맞춘다. `/design/build` 는 급수원이
        #   하나뿐이면 `selected_source=None` 을 넘긴다 — 여기서 "Z1" 을 넘기면
        #   같은 판인데 키가 갈려 **전체망 전개를 두 번** 돈다.
        #   실측(B1F · 급수원 1곳): 그 탓에 선정 92.9s + 표 확정 94.9s 로
        #   같은 계산을 두 번 냈다. 캐시가 조용히 빗나가면 비용만 늘고 아무도
        #   모른다 — 이 저장소가 이미 한 번 겪은 함정이다(attach.py 의 경고).
        probe = wet_heads(
            sess, es,
            selected_source=(picked_tag if len(b.sources) > 1 else None))
        if probe.get("ok") and (probe.get("wet") or None):
            wet = {int(i) for i in probe["wet"]}
            n_before = len(only) if only is not None else len(b.disks)
            only = wet if only is None else (set(only) & wet)
            print(f"[최불리] 전개가 붙일 수 있는 헤드만 후보로 —"
                  f" {n_before} → {len(only)}개"
                  f" (도면 {probe.get('total')} 중 붙는 것 {len(wet)})")
            if not only:
                return None, _wfail(
                    "고른 범위 안에 전개가 배관에 붙일 수 있는 헤드가 "
                    "없습니다 — 손질에서 그 헤드의 배관을 먼저 이어 주세요.")
        else:
            print(f"[최불리] ★붙는 헤드를 못 쟀습니다 — 종전대로 고릅니다"
                  f" ({probe.get('error') or '사유 미상'}). 표 확정에서"
                  f" 모자라면 다음 순위로 채웁니다.")

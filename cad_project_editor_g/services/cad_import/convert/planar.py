...
def main(key=KEY, out=None, *, write=True, pts=None, edges=None, hcov=None,
    ...
    # ★수치 주의 — 「942.7초 중 937.3초」는 **고치기 전** 값이다. 그것을 고친
    #   커밋이 그 문장을 현재형으로 남겨, 읽는 사람이 이 단계를 아직 15분짜리로
    #   믿게 했다. 증분 색인·frozen_geometry 로 O(P) 전수 재구축을 없앤 뒤
    #   재측정: 전체망 942.7초 → 15.0초, B1F 표 확정 1,409초 → 8.6초이고
    #   이 단계는 그중 6.5초다. 여전히 가장 길기에 문구는 그대로 둔다.

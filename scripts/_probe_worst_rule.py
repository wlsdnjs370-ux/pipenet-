...
def main() -> int:
    ...
    with sync_playwright() as p:
        ...
        print("[3b] 설계면적 방식을 골라 본다")
        for rule in ("rect", "branch"):
            pg.select_option("#ed-rule", rule)
            pg.wait_for_timeout(200)
            pg.click("#ed-worst")
            for _ in range(900):
                pg.wait_for_timeout(200)
            ...
            w = pg.evaluate("() => (window.__mf.edit || {}).worst || {}")
            print(f"    {rule:>7}: {w.get('k')}개 · 면적 "
                  f"{w.get('area_w_m')}×{w.get('area_h_m')} m "
                  f"({w.get('area_m2')} ㎡) · corridor {w.get('total_m')} m")
            print(f"             안내 «{pg.inner_text('#ed-rule-why')[:52]}»")
            print(f"             상태줄 {pg.inner_text('#status')[-46:]}")

...
■ 골든 이력 — 왜 움직였는지 적어 둔다 (안 적으면 «덮었다» 로 읽힌다)
  ...
  2026-09-13 · `daemyeong.worst.total_m` **74.02 → 75.78** (손질이 «붙는 헤드»
    만 고르게 된 결과). 움직인 것은 이 한 수뿐이다 — far_m 52.69 · span_m 11.1
    · max_load 10 · 산출물(.kfp 79/78 · 221,571 bytes) 전부 그대로다.

    ★75.78 은 **원래 수리계산 쪽 값**이다. 저장소가 그 어긋남을 이미 적어
      두고 있었다(`design/restrict.py` — 「손질과 수리계산이 서로 다른 K개를
      고른다 · 실측 corridor 총연장 **75.78 대 74.02 m**」). 골든은 그중
      손질 값을 굳혀 두고 있었고, 이제 손질이 표와 같은 선정을 고르므로
      두 수가 하나로 모였다. 시험을 통과시키려 옮긴 값이 아니라
      **두 화면이 같아진 결과**다. 사용자 지시로 기록(2026-09-13).

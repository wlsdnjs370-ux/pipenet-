...
def g13(a_selection):
    ...
    # ★[F-1 재기록] 기준선은 F-1 시점 저장본의 위상이다. 원래는 G9 이전
    #   방출기(9d581f8^)로 떴으나, 저장본이 표류해(08-26 아침 재저장) 그 비교
    #   대상 자체가 사라졌다 — 이후는 이 값 대비 «세로 처리·표시 변경이 위상을
    #   안 건드린다» 를 지킨다.
...
# [G18] 기준선. 최불리 «선정» 값은 지시서 ② §2 그대로여야 한다 — 세로 처리는
#       망을 세로로 세울 뿐 어느 헤드를 고를지에는 영향이 없다.
#
# ★표 관련 수치는 [G19] 에서 **의도적으로** 옮겼다. 그 전(노드 62·배관 61·
#   길이 211.362 m)은 평면 그래프 그대로라 표고가 전부 0 이고 헤드 접속관이
#   아예 없던 망이다. 지금은 `.kfp` 와 같은 세로 처리를 거친다.
# [F-1] 저장본이 08-26 아침 세션에서 재저장돼(3163→3105 헤드) 값이 옮겨갔다 —
# 코드 회귀가 아니라 입력 변화(board 지문·급수원 Z1 규약으로 확인). 지시서 §0.4:
# F-1 이전 산출물은 기준선으로 쓰지 않는다. 아래는 F-1 시점 실측이다.
G18_REFERENCE = {
    "헤드": 30, "최원 유하거리 (m)": 154.25, "설계면적 폭 (m)": 40.21,
    "corridor 총연장 (m)": 207.95, "주배관 담당 헤드 수": 30,
    "노드": 99, "배관": 98, "노즐": 30, "기기": 0, "루프 잔여": 0,
    "관경 근거 — 도면 텍스트": 19, "관경 근거 — 별표1 보강": 0,
    "관경 근거 — 별표1 폴백": 79,
    "부속": 59, "부속 판정 불가": 3, "등가길이 미해결": 0,
}
